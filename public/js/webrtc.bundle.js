...
function WebRTC(opts) {
    ...
    // check for support
    if (!webrtc.support) {
        throw(new Error('Your browser doesn\'t seem to support WebRTC'));
    }
...
Peer.prototype.handleMessage = function (message) {
    ...
    if (message.type === 'offer') {
        ...
        this.pc.answer(message.payload, function (err, sessionDesc) {
            console.log("answer", sessionDesc);
...
Peer.prototype.start = function (mute) {
...
function PeerConnection(config, constraints) {
    ...
    console.log("new PeerConnection", this);
}
...
PeerConnection.prototype.offer = function (constraints, cb) {
    ...
    this.pc.createOffer(
        function (sessionDescription) {
            sessionDescription.sdp = ensureCryptoLine(sessionDescription.sdp);
...
function ensureCryptoLine(sdp) {
    console.log("ensureCryptoLine", navigator.userAgent.match('Firefox'), sdp);
